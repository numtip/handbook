...
| Public runtime data | `site/public/data/manuals.csv` (generated and URL-redacted) |
...
- The public runtime file retains a PDF URL only when `สถานะ` contains `PDF ตรวจพบ`; all other URL cells are blanked before the site is served.
- The generator currently produces 42 records with 9 verified PDF links. Do not edit the generated runtime CSV by hand.
...
6. Confirm only `public` records can expose an external PDF action and that the generated public CSV contains no URL for non-public records.
